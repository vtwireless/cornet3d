...
function startSpectrum(radio, socket, successFunc) {
    ...
    var command = '';
    var logFile = path.join(__dirname, 'cornet3d_spectrumServer_log');
    if(radio.host !== 'localhost')
        command += 'ssh ' + radio.host + ' ';
    command +=
        'nohup ' +
...
function tcpConnectToSpectrum(radio, socket, failFunc) {
	...
	console.log('for ' + socket.Address + ' Connected to spectrumServer'
                + ' on port ' + radio.spectrumPort);
    ...
    tcpClient.on('data', function(data) {
        ...
        socket.send(JSON.stringify({ name: 'updateSpectrum', args: data.data }));
    ...
    tcpClient.on('close', function() {
        console.log('for ' + socket.Address +
