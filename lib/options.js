...
/* Usage: 
   ...
   require(THISFILE).parse(config);

 */

/* config -- is a JSON object that has some values set and this
 *           will get default values from it.
 *
 *   The object returned from this function should be considered the
 *   constant/fixed configuration of this server.  DO NOT CHANGE IT.
 *
 * Returns a new json object that is the union of all the parameters
 * that are in config that was passed in, and the command-line option
 * parameters.
 */
exports.parse = function(config) {
    ...
    // Get the option values into the returned object
    for(var key in options)
        ret[key] = options[key].value;
    // Don't need help now.
    delete ret['help'];

    // Copy the rest of the config to the returned object
    for(var key in config)
        ret[key] = config[key];


    // Figure out what incoming server socket addresses to listen
    // to from the --local command-line option
    if(ret.local) {
        ret.bindAddress = 'localhost';
        console.log('Only localhost clients may connect');
    } else {
        ret.bindAddress = '::';
        console.log('Any address may connect');
    }
    // local is not needed any more.
    delete ret.local;
