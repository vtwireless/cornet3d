...
exports.configDefaults = function(config) {
    ...
    var spectrumPort = checkSet(config, 'spectrumPortStart', 5000);
        ...
        for(var floorName in floors) {
            ...
            if(floor['radios'] !== undefined) {
                floor['radios'].forEach(function(radio) {
                    ...
                    radio['spectrumPort'] = (spectrumPort++).toString();
                    ++radioIndex;
                })
            }
        }
    ...
    // spectrumPortStart is not needed any more.
    delete config.spectrumPortStart;
