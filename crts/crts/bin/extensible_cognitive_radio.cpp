...
ExtensibleCognitiveRadio::ExtensibleCognitiveRadio() {
  ...
  system(systemCMD);
  INFO("Bringing up tun interface\n");
  INFO("Connecting to tun interface\n");
...
CPU_ZERO(&set);
  ...
  */
  ...
  INFO("Initializing USRP settings...\n");
...
ExtensibleCognitiveRadio::~ExtensibleCognitiveRadio() {
  ...
  INFO("destructor signaling ce condition...\n");
  ce_thread_running = false;
  pthread_cond_signal(&CE_cond);

  INFO("destructor joining ce thread...\n");
  void *ce_exit_status;
  ...
  INFO("destructor closing the TUN interface file descriptor\n");
  close(tunfd);

  INFO("destructor bringing down TUN interface\n");
  sprintf(systemCMD, "sudo ip link set dev %s down", tun_name);
  system(systemCMD);

  INFO("destructor deleting TUN interface\n");
  sprintf(systemCMD, "sudo ip tuntap del dev %s mode tun", tun_name);
  system(systemCMD);


  ASSERT(destroyCE, "");
  // Call the CognitiveEngine destructor using the module loader
  // factory function.
  destroyCE(CE);
}
...
void ExtensibleCognitiveRadio::set_ce(char *ce, int argc, char **argv) {
  ...
  char filename[LEN];
  ASSERT(snprintf(filename, LEN, "../plugins/CognitiveEngine/%s.so", ce) < LEN, "");
  ModuleLoader<CognitiveEngine, CognitiveEngine *(*)(int,char**, ExtensibleCognitiveRadio *)> moduleLoader;
  // The RTLD_NODELETE flag is important so this CE object stays mapped
  // after this moduleLoader object is destroyed when this function
  // returns.
  ASSERT(moduleLoader.loadFile(filename, RTLD_NODELETE) == false, "Failed to load DSO plugin \"%s\"", filename);
  CE = moduleLoader.create(argc, argv, this);
  // get the CE object factory destroy() function which will call
  // "delete CE" for us later, after we're done with the CE.
  destroyCE = moduleLoader.destroy;
  ASSERT(destroyCE, "");

  // moduleLoader will cleanup it's self leaving the CE in a usable state.
  // We assume that this is called just once in this program, otherwise
  // we'd have to manage other loaded/created CE objects.
}
...
void ExtensibleCognitiveRadio::start_tx() {
  ...
  switch (tx_worker_state){
    case (WORKER_HALTED):
      INFO("Waiting for tx worker thread to be ready\n");
      wait = true;
      while (wait) {
        pthread_mutex_unlock(&tx_params_mutex);
        usleep(5e2);
    ...
    case (WORKER_READY):
      INFO("Signaling tx worker thread\n");
      pthread_cond_signal(&tx_cond);
      break;
    case (WORKER_RUNNING):
      break;
...
void ExtensibleCognitiveRadio::start_rx() {
  ...
  switch (rx_worker_state){
    case (WORKER_HALTED):
      INFO("Waiting for rx worker thread to be ready\n");
      wait = true;
      while (wait) {
        pthread_mutex_unlock(&rx_params_mutex);
        usleep(5e2);
      ...
      // fall through to signal
    case (WORKER_READY):
      INFO("Signaling rx worker thread\n");
      pthread_cond_signal(&rx_cond);
      break;
    case (WORKER_RUNNING):
      break;
...
void *ECR_rx_worker(void *_arg) {
  ...
  while (ECR->rx_thread_running) {
    ...
    INFO("rx worker received start condition\n");
    ...
    INFO("rx_worker finished running\n");
...
void *ECR_tx_worker(void *_arg) {
  ...
  while (ECR->tx_thread_running) {
    // wait for signal to start
    INFO("tx worker waiting for start condition\n");
    ...
    pthread_mutex_unlock(&ECR->CE_mutex);
    INFO("tx_worker finished running\n");
  } // while tx_thread_running
  INFO("tx_worker exiting thread\n");
  pthread_exit(NULL);
}
