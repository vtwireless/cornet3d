  ...
  if (select(*tcp_controller + 1, &fds, NULL, NULL, &timeout) || (!start_msg_received)) {
    ...
    switch (command_buffer[0]) {
      ...
      case CRTS_MSG_START: // updated start time (used for manual mode)
        NOTICE("Received manual start from controller");
      ...
      case CRTS_MSG_TERMINATE: // terminate program
        NOTICE("Received termination command from controller\n");
        sig_terminate = 1;
        break;
      case CRTS_MSG_CONTROL:
        ...
        NOTICE("Received a %i byte control message\n", rflag);
...
int main(int argc, char **argv) {
  ...
  if (tcp_controller < 0) {
    ERROR("ERROR: Receiver Failed to Create Client Socket\n");
    exit(EXIT_FAILURE);
  }
  ...
  struct scenario_parameters sp;
  NOTICE("Receiving command from controller...\n");
  sleep(1);
  float t_step;
  int fb_enables = 0;;
  ...
  if (np.log_net_tx) {
    ...
    if(np.cognitive_radio_type == EXTENSIBLE_COGNITIVE_RADIO)
    {
        ...
        int argc = 0;
        char ** argv = NULL;
        NOTICE("Converting ce_args to argc argv format\n");
        str2argcargv(np.ce_args, np.cognitive_engine, argc, argv);
        NOTICE("Initializing CR\n");
    ...
    else if(np.cognitive_radio_type == PYTHON)
    {
        ...
        NOTICE("CRTS: Forking child process\n");
        ...
        {
            sleep(5);
            NOTICE("CRTS Child: Initializing python CR\n");
  ...
  if (crts_client_sock < 0) {
    ERROR("CRTS failed to create client socket\n");
    sig_terminate = 1;
  }
  if (crts_server_sock < 0) {
    ERROR("CRTS failed to create server socket\n");
    sig_terminate = 1;
  }
  ...
  while ((time_s < stop_time_s) && (!sig_terminate)) {
    ...
    while (select(crts_server_sock + 1, &read_fds, NULL, NULL, &timeout) > 0) {
      ...
      if (recv_len > 0) {
        ...
        NOTICE("CRTS received packet %i containing %i bytes:\n", rx_packet_num,
                recv_len);
